...
## 4. Статус реализации (актуально)

### 4.1 Реализовано (DONE)
...
- **`llm/system_prompt.txt`** — роль эксперта + плейсхолдер `{{DOMAIN_KNOWLEDGE}}`; фокус на интерпретации в теоретической рамке при строгой опоре на данные.
- **`llm/knowledge_loader.py`** — загрузка domain_briefing и подстановка в системный промпт.
- **`llm/memo_engine.py`** — USER_PROMPTS расширены отсылками к ориентализму/травелогам/эссенциализации.
- **`src/deepseek_analysis.py`** — knowledge-aware системный промпт и явная просьба к теоретической интерпретации.
- **Метаданные документов в payload** — добавлен best-effort парсинг `author/year/title` из `filename` и передача в scientific payload (`document_metadata`).
- **Поэтапный scientific mode** — добавлен staged-режим (несколько вызовов по секциям) через `--scientific-report-staged`.

### 4.2 Частично реализовано (PARTIAL)

- **Связка synthesis ↔ scientific_report**: в scientific payload передаются `prior_interpretations` (memos + synthesis), но нет отдельного шага согласования/оценки конфликтов между блоками.
- **Конвейер научного отчёта**: доступны оба режима — один вызов (A) и staged-вызовы (B), но нет автоматического quality-gate между этапами.

### 4.3 Пока не реализовано (TODO)

- **Полноценная RAG/поиск по внешним источникам**: база знаний пока статическая (`domain_briefing.txt`), без автоматической подкачки и верификации источников.
- **Явная временная аналитика в отчёте**: metadata уже передаётся в payload, но отдельные графики/таблицы «по годам/авторам» в основном отчёте пока не добавлены.

Итог: конвейер уже даёт более «полевой» научный текст (теория + данные), а staged-режим и metadata повышают глубину интерпретации. Дальнейший рост качества — через quality-gates и явные temporal/author sections.
